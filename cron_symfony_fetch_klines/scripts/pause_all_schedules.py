...
# scripts/pause_all_schedules.py
import os, sys, argparse, asyncio
from typing import List, Optional

from temporalio.client import Client

# Reuse env defaults (same as create_all_schedules.py)
TEMPORAL_ADDRESS   = os.getenv("TEMPORAL_ADDRESS", "temporal:7233")
TEMPORAL_NAMESPACE = os.getenv("TEMPORAL_NAMESPACE", "default")

# Optional import of the SCHEDULES dict from create_all_schedules.py to get known IDs
KNOWN_SCHEDULE_IDS = []
try:
    # The file is expected to be importable on PYTHONPATH; if not, we fallback to server listing.
...
async def pause_or_resume_known(client: Client, *, pause: bool, filter_prefix: Optional[str], dry_run: bool) -> None:
    ...
    for sid in targets:
        ...
        if dry_run:
            ...
            continue
        if pause:
            await handle.pause(note="bulk pause_all_schedules")
            print(f"[done] paused {sid}")
        else:
            await handle.unpause(note="bulk pause_all_schedules")
...
async def pause_or_resume_by_listing(client: Client, *, pause: bool, filter_prefix: Optional[str], dry_run: bool) -> None:
    # List schedules from server (requires server & SDK support). Falls back silently if not available.
    try:
        ...
        print("[error] Votre SDK ne supporte pas list_schedules(). Réessayez sans --list en vous basant sur KNOWN_SCHEDULE_IDS (import depuis create_all_schedules.py).")
