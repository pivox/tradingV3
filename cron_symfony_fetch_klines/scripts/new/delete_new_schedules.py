...
# scripts/create_new_schedules.py
...
async def delete_known(client: Client, *, filter_prefix: Optional[str], dry_run: bool) -> None:
    ...
    for sid in targets:
        ...
        if dry_run:
            print(f"[dry-run] DELETE -> {sid}")
            continue
        await handle.delete()
        print(f"[done] deleted {sid}")

async def delete_by_listing(client: Client, *, filter_prefix: Optional[str], dry_run: bool) -> None:
    try:
...
async def main(args: argparse.Namespace) -> None:
    ...
    if args.list:
        await delete_by_listing(client, filter_prefix=args.filter, dry_run=args.dry_run)
    else:
        await delete_known(client, filter_prefix=args.filter, dry_run=args.dry_run)
