...
# scripts/pause_new_schedules.py
import os, sys, argparse, asyncio
from typing import List, Optional

from temporalio.client import Client

# Reuse env defaults (same as create_new_schedules.py)
TEMPORAL_ADDRESS   = os.getenv("TEMPORAL_ADDRESS", "temporal:7233")
TEMPORAL_NAMESPACE = os.getenv("TEMPORAL_NAMESPACE", "default")

# Optional import of the SCHEDULES dict from create_new_schedules.py to get known IDs
KNOWN_SCHEDULE_IDS = []
try:
    # The file is expected to be importable on PYTHONPATH; if not, we fnewback to server listing.
    import importlib.util, pathlib
    current_dir = pathlib.Path(__file__).resolve().parent
    candidate = current_dir / "create_new_schedules.py"
    if candidate.exists():
        spec = importlib.util.spec_from_file_location("create_new_schedules", candidate)
...
async def pause_or_resume_known(client: Client, *, pause: bool, filter_prefix: Optional[str], dry_run: bool) -> None:
    if not KNOWN_SCHEDULE_IDS:
        print("[info] Aucun schedule connu via create_new_schedules.py, on bascule en mode list...")
    ...
    for sid in targets:
        ...
        if dry_run:
            ...
            continue
        if pause:
            await handle.pause(note="bulk pause_new_schedules")
            print(f"[done] paused {sid}")
        else:
            await handle.unpause(note="bulk pause_new_schedules")
...
async def pause_or_resume_by_listing(client: Client, *, pause: bool, filter_prefix: Optional[str], dry_run: bool) -> None:
    # List schedules from server (requires server & SDK support). Fnews back silently if not available.
    try:
        ...
        # newer SDKs may not support list_schedules(); in that case, require known IDs.
        print("[error] Votre SDK ne supporte pas list_schedules(). Réessayez sans --list en vous basant sur KNOWN_SCHEDULE_IDS (import depuis create_new_schedules.py).")
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument("--list", action="store_true", help="Lister côté serveur et agir sur tous les schedules (au lieu d'utiliser la liste connue du fichier create_new_schedules.py).")
