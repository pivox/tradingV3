...
@workflow.defn(name="CronSymfonyMtfWorkflow")
class CronSymfonyMtfWorkflow:
    @workflow.run
    async def run(self, jobs: Iterable[Any]) -> None:
        normalized = _normalize_jobs(jobs)
        for job in normalized:
            workflow.logger.info(f"[CronMtf] Appel MTF: {job.url}")
            try:
                result = await workflow.execute_activity(
                    "call_symfony_endpoint",
                    args=[job.url],  # on ne passe que l'URL à l'activité
                    start_to_close_timeout=timedelta(minutes=5),  # 5 minutes pour l'endpoint MTF
                )
                workflow.logger.info(f"[CronMtf] Réponse {job.url}: {result}")
            except Exception as e:
                workflow.logger.error(f"[CronMtf] Erreur pour {job.url}: {e}")
