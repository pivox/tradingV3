...
`position_trade_analysis_v2` (créée par `Version20260622000000`, durcie par
`Version20260623010000`) apparie une entrée à sa clôture **(1) par `internal_trade_id`
exact, (2) sinon par `trade_id` exact, (3) sinon par `position_id` exact — borné à la même
venue (symbole + exchange + market_type) et à une clôture postérieure à l'entrée —, (4)
sinon `unmatched`** — **jamais** par « première clôture ultérieure du même symbole ».
Pont `trade_id → position_id` via `position_opened` (porte les deux). Une CTE récursive
parcourt chaque groupe en FIFO : une clôture ferme l'entrée ouverte admissible la plus
ancienne, une clôture excédentaire reste orpheline et ne décale pas l'entrée suivante.
Colonnes `close_match_status`, `close_matched_by`, `analysis_status`
(`matched_closed`/`unmatched`).
...
| CLEAN-001 ✅ | Déprécier progressivement le chemin legacy multi-jobs Temporal | Livré : le chemin legacy multi-jobs (`CronSymfonyMtfWorkersWorkflow` / `mtf_api_call` / `MtfJob` / `response_formatter` + les 3 scripts `manage_mtf_workers` / `manage_scalper_micro` / `manage_exchange_profile`) est **marqué déprécié** (CLEAN-001) sans rien supprimer ni casser. Notices `DEPRECATED (CLEAN-001)` en tête des docstrings et dans la description `--help` des 3 scripts ; `DeprecationWarning` runtime émis au lancement de chaque script legacy (helper partagé `utils/legacy_deprecation.py`, ne lève jamais) ; `workflow.logger.warning` déterministe au début de `CronSymfonyMtfWorkersWorkflow.run`. Tous pointent vers la cible unique `scripts/manage_orchestrator_schedule.py` (→ `POST /orchestrator/run`). **Legacy 100 % fonctionnel** (aucun fichier supprimé, aucune signature publique modifiée, worker enregistre toujours les deux chemins). Scripts **actifs** `manage_contract_sync` / `manage_cleanup` **non touchés**. Tests `tests/test_legacy_deprecation.py` (`pytest.warns(DeprecationWarning)` sur les 3 scripts + capture du log workflow), suite cron 100 % verte sans serveur Temporal. **Chemin cible strictement inchangé** ; gate QA-003 `--cov-fail-under=99` toujours vert (legacy hors périmètre `omit`). La suppression effective du legacy est un jalon ultérieur (hors CLEAN-001) ; le guide de migration détaillé = CLEAN-002 (livré, voir ci-dessous). |
| CLEAN-002 ✅ | Documenter la migration depuis le runner legacy multi-jobs | Livré (**documentation uniquement**, aucun code modifié) : guide opérationnel pas-à-pas [Migration legacy → orchestrateur](legacy-migration.md) couvrant le passage du chemin legacy multi-jobs (N `POST /api/mtf/run` par tick) au schedule orchestrateur unique (un `POST /orchestrator/run` par tick). Contenu : principe (où vit désormais la logique — dashboards/sets PY-003/004, concurrence PY-005, persistance PY-006, idempotence/locks SAFE-001/002, garde-fous live SAFE-003, observabilité OBS-001/002), tableau de correspondance des paramètres (`MtfJob.workers`/`dry_run`/`exchange`/`market_type`/`mtf_profile`/… → sets de dashboard + `RunRequest` minimal), procédure pas-à-pas avec commandes exactes (inventaire `status` → préparation dashboard/sets → `manage_orchestrator_schedule.py create --dry-run` puis réel → validation parallèle → `pause`/`delete` legacy), rollback (`resume`/`create`), et échéance de suppression effective des fichiers legacy repoussée à un **jalon FUTUR explicitement hors CLEAN-002**. Les scripts **actifs** `manage_contract_sync` / `manage_cleanup` sont signalés comme **hors migration**. Page ajoutée à `mkdocs.yml` ; renvois depuis `README.md` §0 et `temporal.md`. **Ne supprime rien, ne déprécie rien de plus que CLEAN-001** ; gate QA-003 `--cov-fail-under=99` inchangé (aucun `.py` touché). |
