...
| `scalping` risque par trade non résolu (`7%` defaults contre `2%` risk), caps jour `6%` et `40 USDT`, concurrence `6` | `trade_entry.scalper.yaml:26,73-78`, `TradeEntryRequestBuilder.php:80` | percent equity/trade, percent equity/day, USDT/day, positions | Le conflit du consommateur actif est conservé comme contrainte non résolue; aucune affirmation de performance. |
