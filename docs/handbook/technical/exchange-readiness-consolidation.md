...
| Hyperliquid | Target gateway, dry-run only | Partiel : adapter REST présent, provider bundle skeleton explicite en `not_ready` | Oui, après runtime-check readiness, pas en HL-002 | Non | Oui, `HyperliquidExchangeAdapter` | Oui, `ExchangeProviderBundle.hyperliquid_perpetual` avec gateways skeleton fail-closed | `HYPERLIQUID_ENV`, `HYPERLIQUID_PRIVATE_KEY`, `HYPERLIQUID_ACCOUNT_ADDRESS`, `HYPERLIQUID_API_BASE_URI`, `HYPERLIQUID_WS_URI`, `HYPERLIQUID_MAINNET_ENABLED` | Oui, `Readiness level: not_ready`, `Schedule ready: no` en HL-002 | Config URI présente ; pas de client WS runtime consolidé | Manquant, capability false | Implémenté au niveau adapter, mais le provider skeleton refuse toute exécution | Implémenté au niveau adapter ; provider skeleton refuse | Adapter REST info présent ; provider account skeleton refuse | Adapter REST info présent ; provider account skeleton refuse | Pas attaché sur entrée ; trigger/protection à valider selon modèle Hyperliquid | REST snapshot présent côté adapter ; provider MTF non actif | Partiel via events/reconciliation | Non en HL-002 : schedule reste bloqué par `hyperliquid_provider_bundle_skeleton_not_ready` | Client par défaut indique que le signing live n'est pas activé ; modèle USDC/symboles différent ; presence du bundle ne doit pas impliquer readiness | Implémenter public read-only, account read-only, signer fake, nonce manager, WS ou équivalent, validation protections/reconciliation |
...
| `config/trading/exchange/hyperliquid.yaml` | Confirme `dry_run_runtime_check_only`, `runtime_check_required: true`, `live_enabled: false`. Hyperliquid n'est pas live-ready ; HL-002 ajoute seulement un bundle provider skeleton fail-closed. |
