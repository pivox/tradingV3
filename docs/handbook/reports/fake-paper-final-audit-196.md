...
| 15 | `websocket_disconnect_resync` | Déconnexion après deux événements, blocage avant snapshot, réconciliation locale exacte, acquittement puis reprise de nouveaux événements sans doublon/perte. | `websocketDisconnectResync()` et `testReconnectResumesAfterDeterministicDisconnectWithoutLossOrDuplicate` | PASS — exécutable deux fois depuis fichiers frais |
