...
- Detect a sequential gap only for the `books` stream, where OKX `seqId`/`prevSeqId` proves continuity. Trades and candles have no invented sequence: recovery is allowed only from acknowledged `{source_identity,natural_identity,canonical_digest,overlap_digest}` evidence. Comparisons are canonical inside one origin and use the overlap digest only across REST/WebSocket origins; failure to prove overlap is fatal `market_data_gap_unresolved`.
...
- `OkxPaperStreamFrontier.php`: validated acknowledged `{source_identity,natural_identity,canonical_digest,overlap_digest}` evidence shared by REST/WS recovery.
...
overlap_digest              # same digest except public-trade size, used only across REST/WebSocket origins
```
...
Derive frontiers from raw validated source rows before normalization. `source_identity` is `tradeId` for trades, `<bar>/<opening-ts>` for candles, and `seqId` for books; `natural_identity` includes OKX/native instrument/channel plus that identity; `canonical_digest` hashes the complete canonical market fields and excludes receipt timestamp and REST/WS origin. `overlap_digest` is identical except that public-trade `size_contracts` is excluded because OKX WebSocket may aggregate several REST fills under the final `tradeId`. Use the canonical digest inside one actual origin and the overlap digest only across origins. Control/boundary identities are stable from state/reason plus persisted epoch/sequence. Never use arrival index, ordinal, local time, or a fabricated `prev/seq` for trades/candles.
...
For queued WS data after REST recovery, require the exact natural identity and cross-origin overlap digest for trades/candles before accepting newer rows; within-origin duplicates still require the complete canonical digest. For books only, require `prevSeqId ===` the current materialized `seqId`. Never infer trade/candle continuity from numeric-looking `tradeId`, timestamp adjacency, row count, REST page order alone, or a local ordinal.
...
- Every REST/WS/control event advances its `{source_identity,natural_identity,canonical_digest,overlap_digest}` frontier only on acknowledgement; multi-row frame replay skips only the acknowledged prefix proven by the origin-aware digest policy.
