...
**Goal:** Produce a hash-bound exact net outcome for canonical Backtrader stop and target executions using only PHP-authenticated cost components.

**Architecture:** A focused Python settlement module consumes the already strict `CanonicalBacktestOrderPlan` and immutable execution result. It selects the authenticated stop or target cost branch, validates event lineage and arithmetic, and serializes exact decimal components into a canonical hash-bound result. Runtime integration adds the net outcome only for supported closed traces and fails closed for arbitrary-price exits.

**Tech Stack:** Python 3.12, Pydantic 2.13, Decimal, pytest 8, Backtrader 1.9.78.123.

---

### Task 1: Exact authenticated settlement contract
...
- [ ] Implement frozen dataclasses and `settle_authenticated_outcome(plan, execution)` by selecting the matching target values already authenticated by `plan_hash`; do not multiply any economic rate.
...
- [ ] Write failing tests for stop loss signs, long/short polarity, unsupported holding expiry, non-executed traces, unknown target, forged dataset/plan/config/quantity/stop lineage and non-reconciling authenticated totals.
