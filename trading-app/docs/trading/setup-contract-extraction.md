...
- Crash also mandates legacy `lev_bounds`. The AST retains the condition, while its numeric limits remain an unresolved, fail-closed `mode_or_exchange` dependency rather than becoming setup-owned leverage policy.
...
The PHP loader rejects unknown IDs/versions and identity mismatches without aliases or fallback. The PHP validator and JSON schema reject missing, extra, mistyped and unknown fields/conditions, unsupported timeframes, unknown condition parameters, wrong parameter types, and empty/whitespace strings. Parameter keys are condition-specific in PHP and constrained by equivalent conditional rules in the JSON schema. Critical absent or stale data rejects. Side equality is enforced as `setup.side = context.side = execution.side`. Setup documents cannot own risk budget, leverage caps, or exchange fees; those remain mode/exchange concerns. Unresolved external safety dependencies also reject publication.

Compilation preserves the source `all_of`/`any_of` expression tree in a recursively canonical AST, including execution policy, the three scalper scenario-A MACD alternatives and the redundant crash pullback subset. Its readonly snapshot contains setup version, compatible mode versions, stable configuration hash, condition-catalog state, and provenance indexed by key. A defined catalog hash is a lowercase SHA-256 of the sorted typed condition-ID catalog and must match the catalog supplied to the compiler; unresolved catalogs remain null. When a catalog is supplied, every condition leaf across every nested context, filter, and no-trade branch must be present. Only explicitly unresolved external dependencies such as crash `lev_bounds` are excluded from that coverage check. Duplicate or unknown frozen provenance paths are rejected before compilation. A source artifact outside `config/trading/setup_contract` cannot be loaded or compiled through the canonical loader. Publication also requires a complete condition catalog, executable lifecycle, full trace, and certified net baseline. All eight extracted versions intentionally fail those gates.
