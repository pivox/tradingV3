...
final class MtfBackfillService
{
    ...
    private function processChunk(string $symbol, Timeframe $timeframe, array $chunk): array
    {
        ...
        if (!$this->rateLimiter->waitForToken(1, 5000)) {
    ...
    private function fetchKlinesWithRetry(
        ...
        for ($attempt = 0; $attempt < self::MAX_RETRIES; $attempt++) {
            try {
                ...
                $response = $this->bitmartClient->getKlines(
                    symbol: $symbol,
                    step: $timeframe->getStepInMinutes(),
                    startTime: $start->getTimestamp() * 1000, // BitMart attend des millisecondes
                    endTime: $end->getTimestamp() * 1000
                );

                if (isset($response['data']['klines'])) {
                    return $this->parseKlinesResponse($response['data']['klines'], $symbol, $timeframe);
                }

                throw new \RuntimeException('Invalid response format: ' . json_encode($response));
    ...
    /**
     * Parse la réponse des klines
     */
    private function parseKlinesResponse(array $klinesData, string $symbol, Timeframe $timeframe): array
    {
        $klines = [];
        
        foreach ($klinesData as $klineData) {
            try {
                $kline = new KlineDto(
                    symbol: $symbol,
                    timeframe: $timeframe,
                    openTime: $this->clock->now()->setTimestamp(intval($klineData['open_time']) / 1000),
                ...
                $this->logger->error('[Backfill] Failed to parse kline', [
