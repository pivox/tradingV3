...
final class CanonicalTradeRuntimePolicyValidator
{
    ...
    public static function blockers(TradeEntryConfig $config, bool $fallbackRewriteAttempted = false): array
    {
        $blockers = [];
        ...
        $fallback = $config->getConfig()['fallback_end_of_zone'] ?? null;
        if ($fallbackRewriteAttempted || (is_array($fallback) && ($fallback['enabled'] ?? null) === true)) {
