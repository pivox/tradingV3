...
final class KlineJsonIngestionService
{
     ...
     */
    public function ingestKlinesBatch(array $klines, string $symbol, string $timeframe): IngestionResult
    {
        if (empty($klines)) {
        ...
        $payload = $this->buildJsonPayload($klines, $symbol, $timeframe);
        ...
        try {
            ...
            $this->logger->info('[KlineJsonIngestion] Batch ingested successfully', [
                ...
                'symbol' => is_object($klines[0]) ? $symbol :  'unknown',
                'timeframe' => is_object($klines[0]) ? $timeframe :  'unknown'
            ]);
     ...
     */
    private function buildJsonPayload(array $klines, string $symbol, string $timeframe): array
    {
        return array_map(function($kline) use ($symbol, $timeframe) {
