...
final class Signal1mService extends AbstractSignal
{
    ...
    public function supportsTimeframe(string $tf): bool { return $tf === '1m'; }
    ...
    public function evaluate(Contract $contract, array $klines, array $config): array
    {
        ...
        if (count($klines) < $minBars) {
            ...
            $duration =  (($minBars - count($klines)) + 1) . ' minutes';
    ...
    {
        foreach ($expected as $n) if (!isset($results[$n]) || ($results[$n]['passed']??false)!==true) return false;
        return true;
    }
}
