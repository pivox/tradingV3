...
/**
 * OBS-003 v2 — Durcit `position_trade_analysis_v2` avec un appariement FIFO déterministe.
 *
 * La vue initiale rapprochait les entrées et clôtures par `ROW_NUMBER()` calculé
 * indépendamment côté entrées et clôtures. Si une clôture excédentaire apparaissait entre
 * deux entrées réutilisant la même clé exacte + venue, elle consommait un rang et pouvait
 * laisser l'entrée suivante `unmatched`. Cette migration remplace ce rang indépendant par un
 * parcours FIFO récursif : une clôture ne consomme que l'entrée ouverte admissible la plus
 * ancienne ; une clôture sans entrée ouverte est simplement orpheline et ne décale rien.
 *
 * Ordre des clés exactes :
 *  1. `internal_trade_id` ;
 *  2. `trade_id` legacy documenté ;
 *  3. `position_id` effectif, borné à la même venue.
 *
 * Venue = symbol + exchange + market_type. Le run reste un garde de sécurité : une clôture
 * taguée avec un run différent ne ferme pas l'entrée ; une clôture sans run n'est utilisée que
 * si une seule valeur de run candidate existe pour la clé + venue.
 */
final class Version20260623010000 extends AbstractMigration
{
    public function getDescription(): string
    {
        return 'OBS-003 v2: replace independent row-number close matching by deterministic FIFO matching';
    }
...
SQL);
    ...
    {
        $this->addSql('DROP INDEX IF EXISTS idx_tle_v2_fifo_extra_position');
        $this->addSql('DROP INDEX IF EXISTS idx_tle_v2_fifo_extra_trade');
        $this->addSql('DROP INDEX IF EXISTS idx_tle_v2_fifo_position');
        $this->addSql('DROP INDEX IF EXISTS idx_tle_v2_fifo_internal');

        $previousMigrationClass = __NAMESPACE__ . '\\Version20260622000000';
        if (!class_exists($previousMigrationClass, false)) {
            require_once __DIR__ . '/Version20260622000000.php';
        }
