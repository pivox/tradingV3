...
/**
 * Surface the persisted paper market-data venue in the authoritative v2 analytics view.
 *
 * The venue is provenance, never inferred. It is propagated through each exact FIFO match
 * key with null-safe comparisons so legacy NULL provenance remains matchable and NULL.
 */
final class Version20260719130000 extends AbstractMigration
{
    public function getDescription(): string
    {
        return 'Surface nullable market-data venue provenance in position_trade_analysis_v2.';
    }
...
opened_bridge AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      PARTITION BY NULLIF(e.extra->> 'trade_id', ''), e.symbol, e.exchange, e.market_type, e.market_data_venue
...
entry_resolved AS (
  ...
  LEFT JOIN opened_bridge ob
   ...
   AND ob.market_data_venue IS NOT DISTINCT FROM ee.market_data_venue
   AND ob.rn = 1
),
snapshot_values AS (
  SELECT
...
internal_entry_base AS (
  SELECT id, match_internal_trade_id AS match_key, symbol, exchange, market_type, market_data_venue, run_id, happened_at
  FROM entry_resolved
  WHERE match_internal_trade_id IS NOT NULL
),
internal_close_base AS (
  SELECT c.id, c.match_internal_trade_id AS match_key, c.symbol, c.exchange, c.market_type, c.market_data_venue,
  ...
  JOIN LATERAL (
    ...
    WHERE e.match_key = c.match_internal_trade_id
      ...
      AND e.market_data_venue IS NOT DISTINCT FROM c.market_data_venue
...
internal_events AS (
  SELECT 'entry' AS event_kind, id AS event_id, match_key, symbol, exchange, market_type, market_data_venue, run_id,
         happened_at AS event_time, 0 AS kind_order
  FROM internal_entry_base
  UNION ALL
  SELECT 'close' AS event_kind, id AS event_id, match_key, symbol, exchange, market_type, market_data_venue, run_id,
         effective_close_time AS event_time, 1 AS kind_order
  FROM internal_close_base
),
internal_groups AS (
  SELECT DISTINCT match_key, symbol, exchange, market_type, market_data_venue, run_id FROM internal_events
),
internal_ordered AS (
  SELECT ie.*,
         ROW_NUMBER() OVER (
           PARTITION BY match_key, symbol, exchange, market_type, market_data_venue, run_id
           ORDER BY event_time, kind_order, event_id
         ) AS seq
  FROM internal_events ie
),
internal_walk(match_key, symbol, exchange, market_type, market_data_venue, run_id, seq, open_entry_ids, pair_entry_id, pair_close_id) AS (
  SELECT match_key, symbol, exchange, market_type, market_data_venue, run_id, 0::bigint,
         ARRAY[]::bigint[], NULL::bigint, NULL::bigint
  FROM internal_groups
  UNION ALL
  SELECT w.match_key, w.symbol, w.exchange, w.market_type, w.market_data_venue, w.run_id, o.seq,
         CASE
  ...
  JOIN internal_ordered o
   ...
   AND o.market_data_venue IS NOT DISTINCT FROM w.market_data_venue
...
trade_entry_base AS (
  SELECT id, match_trade_id AS match_key, symbol, exchange, market_type, market_data_venue, run_id, happened_at
...
trade_close_base AS (
  SELECT c.id, c.match_trade_id AS match_key, c.symbol, c.exchange, c.market_type, c.market_data_venue,
         resolved.match_run_id AS run_id, c.effective_close_time
  FROM close_events c
  JOIN LATERAL (
    SELECT CASE
...
trade_groups AS (
  SELECT DISTINCT match_key, symbol, exchange, market_type, market_data_venue, run_id FROM trade_events
),
trade_ordered AS (
  SELECT te.*,
         ROW_NUMBER() OVER (
...
trade_walk(match_key, symbol, exchange, market_type, market_data_venue, run_id, seq, open_entry_ids, pair_entry_id, pair_close_id) AS (
  SELECT match_key, symbol, exchange, market_type, market_data_venue, run_id, 0::bigint,
         ARRAY[]::bigint[], NULL::bigint, NULL::bigint
  FROM trade_groups
  UNION ALL
...
position_entry_base AS (
  SELECT id, eff_position_id AS match_key, symbol, exchange, market_type, market_data_venue, run_id, happened_at
  FROM entry_resolved e
...
position_close_base AS (
  SELECT c.id, c.match_position_id AS match_key, c.symbol, c.exchange, c.market_type, c.market_data_venue,
         resolved.match_run_id AS run_id, c.effective_close_time
  FROM close_events c
  JOIN LATERAL (
    SELECT CASE
...
position_groups AS (
  SELECT DISTINCT match_key, symbol, exchange, market_type, market_data_venue, run_id FROM position_events
),
position_ordered AS (
  SELECT pe.*,
         ROW_NUMBER() OVER (
...
position_walk(match_key, symbol, exchange, market_type, market_data_venue, run_id, seq, open_entry_ids, pair_entry_id, pair_close_id) AS (
  SELECT match_key, symbol, exchange, market_type, market_data_venue, run_id, 0::bigint,
         ARRAY[]::bigint[], NULL::bigint, NULL::bigint
...
SQL);
    ...
    {
        $previousMigrationClass = __NAMESPACE__ . '\\Version20260626000000';
        if (!class_exists($previousMigrationClass, false)) {
            require_once __DIR__ . '/Version20260626000000.php';
        }
        if (!is_a($previousMigrationClass, AbstractMigration::class, true)) {
            $this->throwIrreversibleMigrationException('Previous authoritative v2 analytics view migration is not loadable.');
        }
