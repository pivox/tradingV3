...
final class BalanceSignalDispatcher
{
    ...
    public function __construct(
        ...
        if ($this->endpoint === '') {
            throw new \InvalidArgumentException('BalanceSignalDispatcher endpoint cannot be empty');
        }
    }

    public function dispatch(BalanceSignal $signal): void
    {
        $this->send($signal, 0);
    }

    private function send(BalanceSignal $signal, int $attempt): void
    {
        ...
        if ($body === false) {
            $this->logger->error('[BalanceSignalDispatcher] Failed to encode JSON payload', [
        ...
        try {
            ...
            if (in_array($statusCode, self::SUCCESS_STATUS_CODES, true)) {
                $this->logger->info('[BalanceSignalDispatcher] Signal sent', [
                    'trace_id' => $signal->traceId(),
            ...
            if ($statusCode >= 400 && $statusCode < 500) {
                $this->logger->error('[BalanceSignalDispatcher] Permanent failure', [
                    'trace_id' => $signal->traceId(),
                    'status_code' => $statusCode,
            ...
            $this->logger->warning('[BalanceSignalDispatcher] Temporary failure, will retry', [
                'trace_id' => $signal->traceId(),
                'status_code' => $statusCode,
                'attempt' => $attempt,
                'response' => $responseBody,
            ...
            $this->logger->warning('[BalanceSignalDispatcher] Transport error, will retry', [
                'trace_id' => $signal->traceId(),
                'attempt' => $attempt,
            ...
            $this->logger->error('[BalanceSignalDispatcher] Unexpected error', [
                'trace_id' => $signal->traceId(),
                'attempt' => $attempt,
                'error' => $exception->getMessage(),
            ]);
    ...
    private function retryLater(BalanceSignal $signal, int $attempt): void
    {
        $nextAttempt = $attempt + 1;
        if ($nextAttempt > $this->maxRetries) {
            $this->logger->error('[BalanceSignalDispatcher] Max retries reached', [
                'trace_id' => $signal->traceId(),
            ]);
    ...
    private function persistFailure(BalanceSignal $signal, int $statusCode, string $response): void
    {
